...
namespace TelegramBot.Static.MessageHandlers
{
    public class CryptoPairsMsgHandler : IMyDisposable
    {
        ...
        public async Task EditUserTask(Update update)
        {
            var match = MonitoringTaskCommands.EditPair.Match(update.Message.Text);
            ...
            if (match.Success)
            {
                    ...
                    using (CryptoPairDbHandler dbh = new CryptoPairDbHandler())
                    {
                        ...
                        if (task != null)
                        {
                            if (!string.IsNullOrWhiteSpace(price))
                            {
                                ...
                                {
                                    await BotApi.SendMessage(user.TelegramId, "Error");
                                }
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(pairbase) && !string.IsNullOrWhiteSpace(pairquote))
                    {
                        ...
                        await ListMatchingTasks(pair, user, CallbackDataPatterns.EditPair, strMessage);
                    }
                    else if (!string.IsNullOrWhiteSpace(pairbase))
                    {
                        var strMessage = CultureTextRequest.GetMessageString("CPEditbySymbol", user.Language);
                        await ListMatchingTasks(pairbase, user, CallbackDataPatterns.EditPair, strMessage);
                    }
                    else
                    {
                        await BotApi.SendMessage(user.TelegramId, CultureTextRequest.GetMessageString("CPEditEmpty", user.Language));
                    }
                }
            }
        }

        public async Task EditUserTaskReplyHandler(Update update, UserConfig user)
        {
            ...
            else await BotApi.SendMessage(user.TelegramId, "Pair doesnt exists");
        }

        public async Task EditUserTaskCallbackHandler(Update update)
        {
        ...
        public async Task RemoveUserTaskCallbackHandler(Update update)
        {
        ...
        public async Task RemoveUserTask(Update update)
        {
            var match = MonitoringTaskCommands.DeletePair.Match(update.Message.Text);
            if (match.Success)
            {
                var user = BotApi.GetUserSettings(update).Result;
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(pairbase) && !string.IsNullOrWhiteSpace(pairquote))
                    {
                        ...
                        await ListMatchingTasks(pair, user, CallbackDataPatterns.DeletePair, strMessage);
                    }
                    else if (!string.IsNullOrWhiteSpace(pairbase))
                    {
                        var strMessage = CultureTextRequest.GetMessageString("cryptoPairRemoveBySymbol", user.Language);
                        await ListMatchingTasks(pairbase, user, CallbackDataPatterns.DeletePair, strMessage);
                    }
                    else
                    {
        ...
        public async Task SetSingleTriggerForUserTask(Update update)
        {
            var match = MonitoringTaskCommands.TriggerOncePair.Match(update.Message.Text);
            if (match.Success)
            {
                var id = int.Parse(match.Groups["id"].Value);
                var user = await BotApi.GetUserSettings(update);
        ...
        public async Task AddCommentForTask(Update update)
        {
            var match = MonitoringTaskCommands.AddComment.Match(update.Message.Text);
            if (match.Success)
            {
                var id = int.Parse(match.Groups["id"].Value);
                var user = await BotApi.GetUserSettings(update);
        ...
        public async Task AddCommentForTaskReplyHandler(Update update)
        {
            var user = await BotApi.GetUserSettings(update);
            var msgRegex = RegexHelper.ConvertMessageToRegex(CultureTextRequest.GetMessageString("CPAddComment", user.Language), new List<string>()
            {
        ...
        public async Task CreateTaskFirstStage(Update update, UserConfig user)
        {
            var match = MonitoringTaskCommands.CreatePair.Match(update.Message.Text);
            if (match.Success)
            {
        ...
        public async Task SetPairSymbolStage(Update update, UserConfig user)
        {
        ...
        private async Task SetExchangePStage(Update update, CryptoPair pair, UserConfig user)
        {
            ...
            if (exchangesForPair.Any())
            {
                if (user.SetExchangeAutomaticaly || exchangesForPair.Count == 1)
                {
                    ...
                    if (pair.Price != 0)
                        await SetRaiseOrFallStage(update, pair);
        ...
        /// </summary>
        private async Task SetRaiseOrFallStage(Update update, CryptoPair pair, UserConfig user = default(UserConfig), double price = 0)
        {
            ...
            {
                var lastMsgId = await TelegramUpdatesHelper.GetMessageIdFromUpdateTask(update);
                if (lastMsgId != 0)
            ...
            else
            {
                ...
                await SaveNewTaskToDB(update, user);
            }
        }
        ...
        /// </summary>
        public async Task SetPriceStage(Update update, UserConfig user)
        {
            var pair = GetTempUserTask(update).Result;
            try
            {
        ...
        /// </summary>
        private async Task SaveNewTaskToDB(Update update, UserConfig user)
        {
            var pair = GetTempUserTask(update).Result;
            if (pair?.Price > 0 &&
                ...
                !string.IsNullOrWhiteSpace(pair?.ExchangePlatform))
            {
                ...
                await BotApi.SendMessage(TelegramUpdatesHelper.GetTelegramIdFromUpdate(update).Identifier, formatedmsg, ParseMode.Html);
            }
        }

        #endregion CreateTask stages

        #region DisplayTaskInfo

        public async Task ShowTaskInfo(Update update)
        {
            var match = MonitoringTaskCommands.ShowPair.Match(update.Message.Text);
            if (match.Success)
            {
                var user = BotApi.GetUserSettings(update).Result;
                var strId = match.Groups["id"].Value;
        ...
        public async Task ListMatchingTasks(TradingPair pair, UserConfig user, string datapattern, string Message)
        {
            using (AppDbContext dbContext = new AppDbContext())
            {
                ...
                if (tasks != null && tasks.Any())
                {
                    ...
                    await BotApi.SendMessage(user.TelegramId, Message, replyMarkup: kb);
                }
                else await BotApi.SendMessage(user.TelegramId, CultureTextRequest.GetMessageString("CPCantFindAnyPairsMatching", user.Language));
            }
        }

        public async Task ListMatchingTasks(string SymbolBase, UserConfig user, string datapattern, string Message)
        {
            using (AppDbContext dbContext = new AppDbContext())
            {
        ...
        public async Task ListAllTask(Update update)
        {
            using (AppDbContext dbContext = new AppDbContext())
            {
                ...
                if (user != null)
                {
                    ...
                    if (tasks.Any())
                    {
                        ...
                        await BotApi.SendMessage(update.Message.Chat.Id, strTasks.ToString(), ParseMode.Html);
                    }
                    else await BotApi.SendMessage(update.Message.Chat.Id, CultureTextRequest.GetMessageString("noCryptoTasks", user.Language));
                }
                else await BotApi.SendMessage(update.Message.Chat.Id, CultureTextRequest.GetMessageString("noCryptoTasks", "en"));
            }
        }

        #endregion DisplayTaskInfo

        #region MultiTasksOperations

        public async Task DropEverythingByProcent(Update update)
        {
            var match = MonitoringTaskCommands.ShiftTasks.Match(update.Message.Text);
            if (match.Success)
            {
                var user = await BotApi.GetUserSettings(update);
                ...
                await BotApi.SendMessage(user.TelegramId, "Saved!");
            }
        }

        public async Task FlipTriggeredTasks(Update update)
        {
            using (AppDbContext dbContext = new AppDbContext())
            {
                var user = dbContext.Users.FirstOrDefault(x => x.TelegramId == update.Message.Chat.Id);
